      ...
      listeners: {
        destroy: function(win, opts) {
          ...
          setTimeout(function () {
            if(me.editor) 
            {
              if(!win.closedOverInlineButton && tinymce.isIE) me.editor.selection.moveToBookmark(me.editor.windowManager.bookmark);
              me.editor.focus();
            }
          }, 300);
...
Ext.define('Ext.ux.form.TinyMCETextArea', {
  ...
  */
  ...
  afterRender: function(){
    ...
    if(tabpanel)
    {
      tabpanel.on('tabchange', function(tabPanel, newCard, oldCard, eOpts) {
        ...
        var myCard = me.findParentBy(function(cont, elm) { return cont.getId() == newCard.getId(); });         
        if(myCard)
        {
    ...
    me.on('resize', function(elm, width, height, oldWidth, oldHeight, eOpts) {
      ...
      */
      ...
      else me.syncEditorSize(me.lastWidth, me.lastHeight);
    }, me);     
  },
  //-----------------------------------------------------------------
  ...
  initEditor: function(){
    ...
    // we also do not need the statusbar at the bottom
    me.tinyMCEConfig.theme_advanced_statusbar_location = 'none';
  ...
  focus: function(selectText, delay) {
    ...
    if (delay) {
      ...
      setTimeout(function () { me.focus.call(me, selectText, false); }, delay);
      return me;
    }
  ...
  restoreCurrentSelection: function()
  {
    ...
    if(!me.storedCursorPosition) return;
